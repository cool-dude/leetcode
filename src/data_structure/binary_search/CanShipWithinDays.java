...
the conveyor belt (in the order given by weights). We may
not load more weight than the maximum weight capacity of the ship.

Return the least weight capacity of the ship that will
result in all the packages on the conveyor belt being shipped within D days.
...
ship of capacity 14 and splitting the packages into parts like (2, 3, 4, 5), (1, 6, 7), (8), (9), (10) is not allowed.
Example 2:
...
4th day: 1, 1*/
class Sln{
    boolean isPossible(int D,int capacity, int[] weights){
        int cur=0;
        int days=1,i=0;
    ...
    public int shipWithinDays(int[] weights,int D){
        ...
        while (ll<ul){
            ...
            if(isPossible(D,capacity,weights))
                ul=capacity;
            else
                ll=capacity+1;
        }
