 ...
 * n’th root of x within an error of 0.001 (i.e. suppose the real root is y,
 ...
 * require prior knowledge in numerical analysis (some of them are mentioned here),
...
public class RootOfANumber {
    public static void main(String[] args) {
        double input = 7;
        int n = 3;
        System.out.println(root(input, n));
    }

    static double root(double x, int n) {
        double lower = 0;
        double upper = x;
        return rootHelper(x, lower, upper, n);
    }

    public static double rootHelper(double x, double lower, double higher, int n) {
        //while(lower<upper){
        double temp = (lower + higher) / 2;
        double intermittentVal = Math.pow(temp, n);
        System.out.println(temp + "->" + intermittentVal);

        if (intermittentVal > x) {
            higher = temp;
        } else {
            lower = temp;
        }

        if (Math.abs(x - intermittentVal) < 0.001) {
            System.out.println(temp);
            return temp;
        }
        return rootHelper(x, lower, higher, n);
    }
}
