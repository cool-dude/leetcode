 ...
 * to delete in the two strings in order to get the same string. For instance,
 * the deletion distance between "heat" and "hit" is 3:
 * <p>
 * By deleting 'e' and 'a' in "heat", and 'i' in "hit", we get the string "ht" in both cases.
 * We cannot get the same string from both strings by deleting 2 letters or fewer.
 * Given the strings str1 and str2, write an efficient function
 * deletionDistance that returns the deletion distance between them.
...
class Soln{
    static int utilDeleteDistance(String str1, String str2, int pos1, int pos2){
        if((pos1>str1.length()-1) && (pos2>str2.length-1))
            return 0;
        if(pos1 > str1.length()-1)
            return (str2.length()-pos2);
        else if(pos2 > str2.length()-1)
            return (str1.length()-pos1);
        //case where equal str1[i] == str2[j]
        if (str1.charAt(pos1) == str2.charAt(pos2))
            return utilDeleteDistance(str1, str2, pos1 + 1, pos2 + 1);
            //case where different str1[i] != str2[j]
            //counter is return
        else {
            return Math.min(utilDeleteDistance(str1, str2, pos1 + 1, pos2),
                    utilDeleteDistance(str1, str2, pos1, pos2 + 1)) + 1;
        }
    }

    static int dpApproach(Sting str1,String str2){
        int row = str1.length();
        int col = str2.length();
        if (row == 0)
            return col;
    ...
    static int deleteDistance(String str1,String str2){
        return utilDeleteDistance(str1,str2,pos1,pos2);
    }

    public static void main(String[] args) {
        String str1 = "frog";
        String str2 = "dog";
        System.out.println(deleteDistance(str1, str2));
        System.out.println(dpApproach(str1, str2));

        //System.out.println(dpBottomApproach("", ""));
        System.out.println(dpSoln("sea", "ate"));
        System.out.println(dpSoln("sea", "eat"));
    }
}