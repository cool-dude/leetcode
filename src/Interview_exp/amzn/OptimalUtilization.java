/*Given 2 lists a and b. Each element is a pair of
* integers where the first integer represents the unique
* id and the second integer represents a value. Your task
* is to find an element from a and an element form b such
* that the sum of their values is less or equal to target
* and as close to target as possible. Return a list of ids
...
Explanation:
There are two pairs possible. Element with id = 2 from the list
* `a` has a value 5, and element with id = 4 from the
* list `b` also has a value 5.
Combined, they add up to 10. Similarily, element
* with id = 3 from `a` has a value 7, and element with
* id = 2 from `b` has a value 3.
These also add up to 10. Therefore, the optimal pairs
* are [2, 4] and [3, 2].
*
Example 3:
Input:
