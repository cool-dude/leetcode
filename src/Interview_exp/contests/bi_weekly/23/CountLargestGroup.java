...
* n is grouped according to the sum of its digits.
...
class Sln{
    ...
    public int countLargestGroup(int n){
        ...
        for(int i=1;i<=n;i++){
            ...
            sizeMap.put(sumDigits, sizeMap.getOrDefault(sumDigits,0)+1);
            max=Math.max(sizeMap.get(sumDigits),max);
        }
        int res=0;
        while (Integer k:sizeMap.keySet()){
            if(sizeMap.get(k)==max) res++;
        }
        return res;
    }
}