...
The demons had captured princess (P)
* and imprisoned her in the bottom-right
* corner of a dungeon. The dungeon consists of
* M x N rooms laid out in a 2D grid. Our valiant
* knight (K) was initially positioned in the top-left
* room and must fight his way through the dungeon to rescue the princess.

The knight has an initial health point represented
* by a positive integer. If at any point his health
* point drops to 0 or below, he dies immediately.

Some of the rooms are guarded by demons, so the knight
* loses health (negative integers) upon entering these
* rooms; other rooms are either empty (0's) or contain
...
class Sln2{
    public int calculateMiHP(int[][] dungeon){
        int m=dungeon.length;
        int n=dungeon[0].length;
        int[] dp=new int[n+1];

        //Initialize the matrix to max possible.
        for(int i=0;i<=n;i++)
