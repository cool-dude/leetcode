...
arithmetic if it consists of at
least two elements, and the difference
between every two consecutive elements
is the same. More formally,
a sequence s is arithmetic if
...
class Sln {
    ...
    public List<Boolean> checkArithmeticSubarrays(int[] nums, int[] l, int[] r) {
        ...
        for(int i=0;i<l.length;i++)
            res.add(isAP(Arrays.copyOfRange(nums,l[i],r[i]+1)));
        return res;
    }
}