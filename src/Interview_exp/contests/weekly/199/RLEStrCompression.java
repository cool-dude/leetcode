...
delete at most k characters from s such that
...
will give us "a3bc3d" of length 6. Deleting any of t
he characters 'a' or 'c' would at most decrease the
length of the compressed string to 5, for instance
delete 2 'a' then we will have s = "abcccd" which
compressed is abc3d. Therefore, the optimal way is to
...
class Sln {
    ...
    int f(List<int[]> count, int a, int i, int k) {
        ...
        if (dp[i][k][a] != null) return dp[i][k][a]; // we hit the cache
