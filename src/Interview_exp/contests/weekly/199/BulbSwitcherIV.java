...
by target where target[i] is '1' if the i-th bulb is
turned on and is '0' if it is turned off.

You have a switch to flip the state of the bulb, a
flip operation is defined as follows:
...
class Sln {
    public int minFlips(String target) {
        int n=target.length();    //total bulbs
        int flips=0;
        char stat='0';          //stores the state of bulbs
        for(int i=0;i<n;i++){
            if(stat!=target.charAt(i)) {
                flips++;
                stat=stat=='0'?'1':'0';
            }
        }
