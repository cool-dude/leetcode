...
public class Solution {
    ...
    */
    ...
    public static boolean hasCommonAncestor(int[][] A,int x,int y){
        ...
        for(int[] num:A){
            Set<Integer> parentSet=ancestorMap.getOrDefault(num[1],new HashSet<>());
            parentSet.add(num[0]);
            ancestorMap.put(num[1],parentSet);
            if(!ancestorMap.containsKey(num[0])){
                ancestorMap.put(num[0],new HashSet<>());
                continue;
            }
            ancestorMap.get(num[1]).addAll(ancestorMap.get(num[0]));
        }
