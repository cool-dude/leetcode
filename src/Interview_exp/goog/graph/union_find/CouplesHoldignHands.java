...
There is an alternative way for building the index groups
 ...
 the cycle like i0 --> i1 --> ... --> jk --> i0, we can
 also build it like i0 <-- i1 <-- ... <-- ik <-- i0, where
 i <-- j means the element at index j is expected to appear
 at index i. In this case, the pivot index will be changing
 along the cycle as the swapping operations are applied.
 The benefit is that we only need to do swapping on the row array.*/
class Sln2 {
