...
In a row of trees, the i-th tree
produces fruit with type tree[i].
...
SLN:Find out the longest length of subarrays with at most 2 different numbers?*/
class Sln{
    public int totalFruit(int[] tree){
        ...
        for(int j=0;j<tree.length;j++){
            ...
            while (cnt.size()>2){
                ...
                if(cnt.get(tree[i])==0) cnt.remove(tree[i]);
                i++;
            }
